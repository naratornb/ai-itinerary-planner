...
Phase 1 (current): the backend is **Flask 3.0.3**, a working prototype covering Supabase auth and basic user admin. Phase 2 trigger: **before the first `/ai/*` endpoint lands**, the entire app is ported to **FastAPI** in a single focused migration — async behaviour fits the LangChain RAG / GPT-4o pipeline (NFR-P-101: < 10s end-to-end) and Pydantic validation matches the strictly-typed contracts in `apps/api/openapi.yaml`.
...
- `apps/api/openapi.yaml` describes the **Phase 2** target (FastAPI, port 8000, `/v1` prefix). Phase 1 code runs on port 5000 with no prefix. New Flask endpoints should adopt the *contract shape* from the spec (paths, payloads, JWT auth, whole-AUD-dollar money) even though the framework differs.
